...
def train(train_loader,train_meta_loader,model,model_ema,vnet,optimizer_model,optimizer_vnet,epoch,meta_lr,flag):
    print('\nEpoch: %d' % epoch)
    ...
    for batch_idx, (inputs, targets, index) in enumerate(train_loader):
        ...
        if batch_idx % 20 == 0:
            if flag:
                ...
                cost_1 = torch.sum(-F.log_softmax(outputs, dim=1) * mix_targets, dim=1)
                cost_11 = torch.reshape(cost_1, (len(cost_1), 1))
                v_lambda_1 = vnet(cost_11.data, targets.data, c).squeeze(1)
                v_lambda_2 = vnet(cost_11.data, targets[idx].data, c).squeeze(1)
                mix_v_lambda = l * v_lambda_1 + (1-l) * v_lambda_2

                l_f_meta = ( ( torch.sum(-F.log_softmax(outputs, dim=1) * mix_targets, dim=1) * mix_v_lambda )
                            +( torch.sum(-F.log_softmax(outputs, dim=1) * mix_psudo_label, dim=1) * (1-mix_v_lambda) )
                           ).mean()
        ...
        loss = ( ( torch.sum(-F.log_softmax(outputs, dim=1) * mix_targets, dim=1) * mix_v_lambda )
                +( torch.sum(-F.log_softmax(outputs, dim=1) * mix_psudo_label, dim=1) * (1-mix_v_lambda) )
                ).mean()
...
def main():
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # torch.save(cot.state_dict(), '/data/save/net_%s_%d.pth' % (args.dataset, epoch))
        # torch.save(cot_ema.state_dict(), '/data/save/net_ema_%s_%d.pth' % (args.dataset, epoch))
        # torch.save(vnet.state_dict(), '/data/save/vnet_%s_%d.pth' % (args.dataset, epoch))
        # torch.save(get_label.soft_labels, '/data/save/soft_labels_%s_%d.pth' % (args.dataset, epoch))
        ...
        print('------\n'
              ...
              'Epoch Runtime: [{:6.2f}]'
              '\n------'.format(
               epoch + 1, num_epochs, train_acc, test_acc_ema,
               epoch_end_time - epoch_start_time))
