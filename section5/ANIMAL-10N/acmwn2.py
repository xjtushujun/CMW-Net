...
def main(arg_seed):
    ...
    for epoch in range(20, num_epoch):
        ...
        metadataset = get_dataset(net, trainloader)
        ...
        for i, (inputs, targets, index) in enumerate(trainloader):
            ...
            if i % 10 == 0:
                ...
                v_lambda = vnet(cost_11.detach().data, targets.detach().data, c).squeeze(1)

                l_f_meta = ( F.cross_entropy(outputs, targets, reduce=False) * v_lambda ).mean()
            ...
            loss = ( F.cross_entropy(outputs, targets, reduce=False) * v_lambda ).mean()

            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

            my_wl(epoch_loss=cost_1, epoch_weight=v_lambda, index=index)
