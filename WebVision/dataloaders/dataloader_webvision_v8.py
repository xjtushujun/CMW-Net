...
class webvision_dataset(Dataset):
    def __init__(self, root_dir, transform, mode, num_class, num_samples=None, losses=[], all_targets=[]):
        self.root = root_dir
        self.transform = transform
        self.mode = mode
            ...
            if self.mode == 'all':
                ...
                if self.mode == "meta":
                    ...
                    for j in range(num_class):
                        data_list[j] = [i for i, label in enumerate(all_targets) if label == j]
                    ...
                    self.meta_targets = [all_targets[i] for i in idx_to_meta]
    ...
    def __getitem__(self, index):
        if self.mode == 'meta':
            ...
            target = self.meta_targets[index]
...
class webvision_dataloader():
    ...
    def run(self, mode, losses=[], all_targets=[]):
        if mode == 'warmup':
        ...
        if mode == 'warmup_sta':
            ...
            meta_dataset = webvision_dataset(root_dir=self.root_dir, transform=self.transform_train, mode="meta",
                                                num_class=self.num_class, losses=losses, all_targets=all_targets)
