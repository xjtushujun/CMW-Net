...
class mnist(data.Dataset):
    ...
    def __init__(self, root, train_or_not=True, download=False, transform=None, target_transform=None,
                 partial_type='binomial', partial_rate=0.1, random_state=0, index=None):
        ...
        if self.train:
            ...
            print('type:', type(self.train_data), type(self.train_labels))
            print('shape:', self.train_data.shape, self.train_labels.shape)
    
            # self.train_data = (self.train_data)[index].squeeze(0)
            # self.train_labels = (self.train_labels)[index].squeeze(0)
            # self.train_final_labels = self.train_labels
            _, train_final_labels = torch.max( torch.load('./save/%s_%f/labels_%d.pth' % ('mnist', partial_rate, 40)), dim=-1)

            mask = (self.train_labels == train_final_labels)

            self.train_data = self.train_data[mask]
            self.train_final_labels = train_final_labels[mask]
            self.train_labels = self.train_labels[mask]

            print('shape:', self.train_data.shape, self.train_final_labels.shape, self.train_labels.shape)

            # self.train_final_labels = (train_final_labels)[index].squeeze(0)
