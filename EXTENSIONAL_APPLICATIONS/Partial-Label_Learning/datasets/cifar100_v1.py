...
class cifar100(data.Dataset):
    ...
    def __init__(self, root, train_or_not=True, download=False, transform=None, target_transform=None,
        ...
        if self.train:
            ...
            self.train_data = torch.from_numpy(self.train_data)[index].squeeze(0)
            self.train_labels = torch.tensor(self.train_labels, dtype=torch.long)[index].squeeze(0)
            # self.train_final_labels = self.train_labels
            _, train_final_labels = torch.max( torch.load('./save/%s_%f/labels_%d.pth' % ('cifar100', partial_rate, 40)), dim=-1)
